...
export function Dashboard() {
  ...
  const pieData = analyticsData?.tasksCompletedByGoal.filter(g => g.count > 0) || [];

  // Prepare data for bar chart (by day)
  const barData = analyticsData?.tasksCompletedByDay.map(d => ({
    name: formatDayOfWeek(d.date),
    count: d.count,
  })) || [];

  // Prepare data for line chart (by hour)
  const lineData = analyticsData?.tasksCompletedByHour.map(h => ({
    name: formatHour(h.hour),
    count: h.count,
  })) || [];

  // Colors for pie chart
  const COLORS = ['#9b87f5', '#1EAEDB', '#0EA5E9', '#7E69AB', '#FF6B6B', '#4CAF50', '#FFA726', '#26C6DA'];

  // Determine longest streak
  const maxStreak = goals.reduce((max, goal) => Math.max(max, goal.stats.streak), 0);
    ...
    <div className="p-6 overflow-y-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-3xl font-bold">{analyticsData?.tasksCompletedToday || 0}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-3xl font-bold">{analyticsData?.tasksCompletedThisWeek || 0}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            {streakGoal && maxStreak > 0 && (
              <p className="text-sm text-muted-foreground">on {streakGoal.title}</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            {loading ? (
              ...
              <>
                {analyticsData && analyticsData.tasksCompletedByHour.length > 0 ? (
                  <p className="text-3xl font-bold">
                    {formatHour(
                      analyticsData.tasksCompletedByHour.reduce(
                        (maxHour, current) => 
                          current.count > analyticsData.tasksCompletedByHour[maxHour].count 
                            ? current.hour 
                            : maxHour,
                        0
                      )
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card className="h-[400px]">
          <CardHeader>
            ...
            <CardDescription>Distribution of completed tasks across goals</CardDescription>
          </CardHeader>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    dataKey="count"
                    nameKey="goalTitle"
                    label={({ goalTitle, percent }) => `${goalTitle}: ${(percent * 100).toFixed(0)}%`}
                  >
        ...
        <Card className="h-[400px] lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>When you're most productive throughout the day</CardDescription>
          </CardHeader>
          <CardContent className="h-[300px]">
