...
// Get goal statistics
export const getGoalStatistics = async (goalId: string): Promise<{
  total: number;
  completed: number;
  percentage: number;
  streak: number;
...
export const getGoalsWithStatistics = async (): Promise<(Goal & {
  stats: {
    total: number;
    completed: number;
    percentage: number;
...
export const getTasksForAnalysis = async (): Promise<{
  ...
  const completedTasks = tasks.filter(task => task.completed && task.completionTimestamp);
  
  // Tasks completed today
  const tasksCompletedToday = completedTasks.filter(
    task => task.completionTimestamp && task.completionTimestamp >= today.getTime()
  ).length;
  
  // Tasks completed this week
  const tasksCompletedThisWeek = completedTasks.filter(
    task => task.completionTimestamp && task.completionTimestamp >= startOfWeek.getTime()
  ).length;
  ...
  const tasksCompletedByGoal = goals.map(goal => {
    const count = completedTasks.filter(task => task.goalId === goal.id).length;
...
export const resolveGoalId = async (goalName: string): Promise<string | null> => {
  const goals = await db.getGoals();
