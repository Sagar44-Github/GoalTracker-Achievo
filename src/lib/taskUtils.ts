...
export const suggestTags = (title: string): string[] => {
  ...
  if (
    /\b(email|meeting|call|presentation|report|deadline|client|project|workflow|work)\b/.test(
      title_lower
    )
  ) {
  ...
  if (
    /\b(study|exam|test|quiz|homework|assignment|class|course|lecture|read|book|notes|research|essay|paper|thesis)\b/.test(
      title_lower
    )
  ) {
  ...
  if (
    /\b(grocery|shopping|clean|laundry|cook|meal|exercise|gym|doctor|appointment|family|friend|call|visit|party|event)\b/.test(
      title_lower
    )
  ) {
  ...
  if (
    /\b(gym|workout|exercise|run|jog|swim|health|doctor|dentist|medicine|pill|vitamin|diet|nutrition|sleep|rest|meditate|yoga)\b/.test(
      title_lower
    )
  ) {
  ...
  if (
    /\b(pay|bill|budget|finance|money|bank|invest|tax|expense|income|salary|debt|loan|mortgage|rent|insurance)\b/.test(
      title_lower
    )
  ) {
  ...
  if (
    /\b(travel|trip|flight|hotel|vacation|booking|reservation|passport|visa|pack|luggage|map|itinerary|tour|guide)\b/.test(
      title_lower
    )
  ) {
...
/**
 * Format a date string in a more user-friendly way
 * @param dateStr The date string in ISO format (YYYY-MM-DD)
 * @returns Formatted date string or "No date" if no date provided
 */
export const formatDate = (dateStr: string | null): string => {
  ...
  // Check if date is in the near future
  if (date > tomorrow) {
...
export const parseCommand = (
  ...
  if (addRegex.test(command)) {
    ...
    const dateRegex =
      /(today's|tomorrow's|this week's|next week's|overdue)\s+tasks/i;
